...
boolean automatique;//declaration bouton reglage automatique
...
Serial.begin(9600);//initialisation de l'horloge serie à 9600
  ...
  positionH_enregistree = 90;//initialisation des variables de position à 90 (position initiale)
  Servo_vertical.write(position_verticale);
  Servo_horizontal.write(position_horizontale);//mise des servomoteurs en position initiale
}

void loop() {
  bouton_droite = digitalRead(4);//lecture valeur bouton_droite sur broche 4
  Serial.print(bouton_droite);
  if ((bouton_droite==1)&&(position_horizontale<(118))) {//si bouton_droite appuye et position max non atteinte
  ...
  if ((bouton_gauche==1)&&(position_horizontale>(62))) {//si bouton RH appuye et position min non atteinte
    position_horizontale=position_horizontale-1;//incrementer position_horizontale
    Servo_horizontal.write(position_horizontale);//mettre servomoteur vertical à la position "position_horizontale"
    bouton_gauche = digitalRead(5);
  }
   delay(45);//attente pour reguler la vistesse de rotation
   bouton_haut = digitalRead(2);//lecture valeur bouton_haut sur broche 2
 Serial.print(bouton_haut);
  if ((bouton_haut==1)&&(position_verticale<157)) {//si bouton_haut appuye et position max non atteinte
  ...
  bouton_bas = digitalRead(3);//lecture valeur bouton_bas sur broche 3
  if ((bouton_bas==1)&&(position_verticale>33)) {//si bouton retro_bas appuye et position min non atteinte
    position_verticale--;//decrementer position_verticale
    Servo_vertical.write(position_verticale);//mettre servomoteur vertical à la position "position_verticale"
    bouton_bas = digitalRead(3);
  }
  delay(49);//attente pour reguler la vistesse de rotation (94-45)
  
   memorisation = digitalRead(8);//lecture valeur bouton memorisation sur la broche 8
   if (memorisation==1) {//si bouton memorisation appuye
    positionV_enregistree = position_verticale;
    positionH_enregistree = position_horizontale;//enregistrement de la position actuelle des deux moteurs
    memorisation = digitalRead(8);
   }
   automatique = digitalRead(9);//lecture valeur bouton automatique sur la broche 9
   if (automatique==1) {//si bouton automatique appuye
    ...
    position_horizontale = positionH_enregistree ;//reglage des retroviseurs à la position enregistree
    rotation_autoV = abs(positionV_enregistree-position_verticale);
    rotation_autoH = abs(positionH_enregistree-position_horizontale);//calcul de l'angle à effectuer
      if (rotation_autoV>rotation_autoH) {//si angle vertical > angle horizontal
      delay((rotation_autoV*720)/360);//attendre le temps que le moteur vertical se règle
    }
    else {//sinon
      delay((rotation_autoH*720)/360);//attendre le temps que le moteur horizontal se règle
    }
